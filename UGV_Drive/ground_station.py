    ...
    try:
        while True:
            ...
            if cmd:
                ...
                elif cmdVal == v2v_bridge.CMD_TURN_ANGLE_RIGHT:
                    # parametric right turn: cmdSeq carries the angle in whole degrees
                    # this command is sent by ugv_navigator.py for visual-servo alignment
                    angle_deg = float(cmdSeq)  # extract the requested angle from the sequence field
                    if not vehicle.armed: arm_and_move(bridge)
                    print(f"[Ground] ANGLE TURN RIGHT: {angle_deg:.1f} deg (from navigator)")
                    execute_turn(bridge, angle_deg)  # positive degrees = clockwise = right
                elif cmdVal == v2v_bridge.CMD_TURN_ANGLE_LEFT:
                    # parametric left turn: cmdSeq carries the angle in whole degrees
                    # this command is sent by ugv_navigator.py for visual-servo alignment
                    angle_deg = float(cmdSeq)  # extract the requested angle from the sequence field
                    if not vehicle.armed: arm_and_move(bridge)
                    print(f"[Ground] ANGLE TURN LEFT: {angle_deg:.1f} deg (from navigator)")
                    execute_turn(bridge, -angle_deg)  # negative degrees = counter-clockwise = left
                elif cmdVal == v2v_bridge.CMD_MOVE_DIST:
                    # parametric drive: cmdSeq carries the distance in centimetres
                    # this command is sent by ugv_navigator.py for visual-servo approach
                    dist_m = float(cmdSeq) / 100.0  # convert centimetres → metres for execute_drive
                    if not vehicle.armed: arm_and_move(bridge)
                    print(f"[Ground] MOVE DIST: {dist_m:.3f}m (from navigator)")
                    execute_drive(bridge, dist_m)  # drive the requested distance
                elif eStopFlag == 1:
