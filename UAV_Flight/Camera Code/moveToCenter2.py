...
def moveToCenter(master, UAVcommander, estimator, cam, args, bottomRight=True):
    ...
    try:
        ...
        climb_step = 0.1  # meters per loop (slow climb)
        ...
        for step in range(steps):
            ...
            # ✅ If both markers found → STOP and hover immediately
            if pair is not None:
                ugv_pose, dest_pose = pair
            ...
            # ❌ Not found → keep moving + climbing
        ...
        # After reaching center, keep climbing & searching
        while True:
            frame = cam.get_frame()
            ...
            if pair is not None:
                UAVcommander.log_event("Markers found after reaching center → hovering")
                return 1
